...
public:
	...
	void run() {
		if (algorithm_flag == 0)
			result = bilateral_normal_filtering(data->output_mesh, iteration_step_size, n_vertex_iterations, sigma_c_ratio, sigma_s);
		else if (algorithm_flag == 1)
			result = guided_denoising(data->output_mesh, iteration_step_size, n_vertex_iterations, sigma_c_ratio, sigma_s, 1.5f);
		else if (algorithm_flag == 2)
			result = uniform_laplacian_smoothing(data->output_mesh, 1, 0.5);
		else
			result = bilateral_normal_filtering(data->output_mesh, iteration_step_size, n_vertex_iterations, sigma_c_ratio, sigma_s);
		finishThread();
	}
